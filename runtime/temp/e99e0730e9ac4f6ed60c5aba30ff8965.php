<?php if (!defined('THINK_PATH')) exit(); /*a:5:{s:38:"template/platform/Config/linkList.html";i:1581654355;s:27:"template/platform/base.html";i:1581654383;s:44:"template/platform/controlCommonVariable.html";i:1581654383;s:27:"template/platform/page.html";i:1581654384;s:33:"template/platform/openDialog.html";i:1581654383;}*/ ?>
<!DOCTYPE html>
...
//查询用户列表
	function LoadingInfo(pageIndex) {
		...
		$.ajax({
			type : "post",
			url : "PLATFORM_MAIN/config/linklist",
			async : true,
			data : {
			...
			success : function(data) {
				...
				if (data['data'].length > 0) {
					for (var i = 0; i < data['data'].length; i++) {
						...
						html += '<td class="center"><a href="PLATFORM_MAIN/config/updatelink?link_id=' + data['data'][i]["link_id"] + '">编辑</a> ';
	...
	$( "#dialog" ).dialog({
        buttons: {
            "确定": function() {
                ...
                $.ajax({
    				type:"post",
    				url:"PLATFORM_MAIN/config/dellink",
    				data:{'link_id':link_id},
    				async:true,
    				success:function (data) {
    					if(data['code'] > 0){
    						location.href = "PLATFORM_MAIN/config/linklist";
    					}else{
    						$("#dialog").dialog({
    							 buttons: {
    					                "确定,#e57373": function() {
