<?php if (!defined('THINK_PATH')) exit(); /*a:5:{s:39:"template/platform/Config/blockList.html";i:1581654354;s:27:"template/platform/base.html";i:1581654383;s:44:"template/platform/controlCommonVariable.html";i:1581654383;s:27:"template/platform/page.html";i:1581654384;s:33:"template/platform/openDialog.html";i:1581654383;}*/ ?>
<!DOCTYPE html>
...
//查询用户列表
	function LoadingInfo(pageIndex) {
		...
		$.ajax({
			type : "post",
			url : "PLATFORM_MAIN/Config/blockList",
			async : true,
			data : {
			...
			success : function(data) {
				...
				if (data['data'].length > 0) {
					for (var i = 0; i < data['data'].length; i++) {
						...
						html += '<td class="center"><a href="PLATFORM_MAIN/Config/updateBlock?block_id=' + data['data'][i]["block_id"] + '">编辑</a>&nbsp;&nbsp;<a href="javascript:;" onclick="delBlock(' + block_id + ')">删除</a></td> ';
						html += '</tr>';
					}
				} else {
	...
	$( "#dialog" ).dialog({
        buttons: {
            "确定": function() {
                ...
                $.ajax({
        			type : "post",
        			url : "PLATFORM_MAIN/config/delblock",
        			async : true,
        			data : {
        				"block_id" : block_id
        			},
        			success : function(data) {
        				if(data["code"] > 0 ){
        					showMessage('success', data["message"],'PLATFORM_MAIN/config/blocklist');
        				}else{
...
function setWebBlockIsBlock(eve,block_id,is_block){
	$.ajax({
		type : "post",
		url : "PLATFORM_MAIN/config/setwebblockisblock",
		async : true,
		data : {
			"block_id" : block_id,
			"is_display" : is_block
