<?php if (!defined('THINK_PATH')) exit(); /*a:5:{s:37:"template/admin/Goods/recycleList.html";i:1581654293;s:24:"template/admin/base.html";i:1581654310;s:41:"template/admin/controlCommonVariable.html";i:1581654310;s:30:"template/admin/pageCommon.html";i:1581654310;s:30:"template/admin/openDialog.html";i:1581654310;}*/ ?>
<!DOCTYPE html>
<html>
	<head>
...
<table class="mytable">
	<tr>
		<th style="line-height:33px;">
			商品分类：
			<input type="text" placeholder="请选择商品分类" id="goodsCategoryOne">
			<div class="goodsCategory one">
				<ul>
					...
					<li onclick="goodsCategoryOne(this);" category_id="<?php echo $vo['category_id']; ?>"><?php echo $vo['category_name']; if($vo['is_parent'] == 1): ?>
			...
			<tbody>
				...
				<tr class="trcss">
					<td colspan="5">
						...
						<a class="btn btn-small fun-a" href="javascript:batchRegainDelete()">批量恢复</a> 
...
// 						}
						...
						html += '<a href="javascript:;" data-placement="bottom" data-original-title="删除"><span class="edit" style="display: inline-block; width: 19%;" onclick="deleteGoods('
							+ data["data"][i]["goods_id"]
							+ ')"><i class="icon-edit" style="width: initial;"></i>删除</span></a>';
						html += '<a href="javascript:void(0)" data-placement="bottom"onclick="regainGoodsDeleted('
							+ data["data"][i]["goods_id"]
							+ ')" data-original-title="恢复"><span class="del" style="display: inline-block; width: 19%;"><i class="icon-trash" style="width: initial;"></i>恢复</span></a></div></div></td></tr>';
					}
				} else {
...
//批量删除回收站数据
	...
	function goodsCategoryOne(obj){
		parentId = $(obj).attr("category_id");
		category_name = $(obj).text();
		$(".one ul li").not($(obj)).removeClass("selected");
				...
				if(data.length>0){
					html = ''
						for (var i = 0; i < data.length; i++) {
							html += '<li onclick="goodsCategoryTwo(this);" category_id="'+data[i]['category_id']+'">'+data[i]['category_name'];
					...
					$(".two").show();
					$(".selectGoodsCategory").css("width",'434px');
				}else{
					$(".one").hide();
					$(".two").hide();
	...
	function goodsCategoryTwo(obj){
		var parentId = $(obj).attr("category_id");
		var category_name = $(obj).text();
		$(".two ul li").not($(obj)).removeClass("selected");
		$(obj).addClass("selected");
		...
		$.ajax({
			...
			success : function(data){		
				if(data.length>0){
					...
					$(".three").show();
					$(".selectGoodsCategory").css("width",'516px');
				}else{
					$(".one").hide();
					$(".two").hide();
					$(".three").hide();
