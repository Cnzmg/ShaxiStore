<?php if (!defined('THINK_PATH')) exit(); /*a:5:{s:37:"template/admin/Config/shopAdList.html";i:1581654288;s:24:"template/admin/base.html";i:1581654310;s:41:"template/admin/controlCommonVariable.html";i:1581654310;s:30:"template/admin/pageCommon.html";i:1581654310;s:30:"template/admin/openDialog.html";i:1581654310;}*/ ?>
<!DOCTYPE html>
<html>
	<head>
...
<?php if($list['total_count'] < 5): ?>
<li><a class="nscs-table-handle_green" href="ADMIN_MAIN/config/addshopad"><i class="fa fa-plus-circle"></i>&nbsp;添加广告</a></li>
<?php endif; ?>
  ...
  <div class="mb10">
       <div class="tiphelp-info"><strong>提示：</strong>设置广告管理，设置方法如下：<br>
                    最多可添加五张轮播图。
       </div>
 </div>
...
//查询用户列表
	function LoadingInfo(pageIndex) {
		...
		$.ajax({
			type : "post",
			url : "ADMIN_MAIN/config/shopadlist",
			async : true,
			data : {
			...
			success : function(data) {
				...
				if (data['data'].length > 0) {
					for (var i = 0; i < data['data'].length; i++) {
						...
						html += '<td><img src="__UPLOAD__/'+ data['data'][i]['ad_image']+'" style="height:50px;"/></td>';
						...
						html += '<td><a href="ADMIN_MAIN/config/updateshopad?id=' + data['data'][i]["id"] + '">编辑</a> ';
	...
	function delShopAd(id){
		$.ajax({
			type : "post",
			url : "ADMIN_MAIN/config/delshopad",
			data : {
				'id' : id
			},
			async : true,
			success : function(data) {
				if (data["code"] > 0) {
					showMessage('success', "店铺广告删除成功",'ADMIN_MAIN/config/shopadlist');
				}else{
