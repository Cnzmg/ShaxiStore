<?php if (!defined('THINK_PATH')) exit(); /*a:5:{s:44:"template/platform/Complain/complainList.html";i:1581654352;s:27:"template/platform/base.html";i:1581654383;s:44:"template/platform/controlCommonVariable.html";i:1581654383;s:27:"template/platform/page.html";i:1581654384;s:33:"template/platform/openDialog.html";i:1581654383;}*/ ?>
<!DOCTYPE html>
...
<div class="row padder-v">
	...
	<div style="width: 17%;float: left;">
		<div style="width:30%;float:left;line-height: 30px;">投诉主题</div>
		<select style="width: 70%;height: 30px;border-color: #d9d9d9;" class="input-sm form-control" id="complain_subject">
			<option value="">全部</option>
			<?php if(is_array($complainSubjectList) || $complainSubjectList instanceof \think\Collection || $complainSubjectList instanceof \think\Paginator): $i = 0; $__LIST__ = $complainSubjectList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
			<option value="<?php echo $vo['complain_subject_id']; ?>"><?php echo $vo['complain_subject_content']; ?></option>
			<?php endforeach; endif; else: echo "" ;endif; ?>
		</select>
	</div>
	<input type="text" id='startDate' placeholder="请选择开始日期" class="input-medium" style="float:left;width:203px;margin-left:5px;margin-right:5px;padding:5px 10px;height:30px;border:1px solid #d9d9d9;" onclick="WdatePicker()"/>
...
$(function(){
	...
	function LoadingInfo(pageIndex) {
		...
		var complain_state=<?php echo $complain_state; ?>;
		$.ajax({
			type : "post",
			url : "PLATFORM_MAIN/complain/ajaxComplainList",
			async : true,
			data : {
				"pageIndex" : pageIndex,
			...
			success : function(data) {
				...
				if (data["data"].length > 0) {
					...
					for (var i = 0; i < data["data"].length; i++) {
						...
						html += '<td class="center">' + data["data"][i]["accused_shop_name"] + '</td>';
						...
						html += '<td class="center"><a href="PLATFORM_MAIN/complain/complainProgress?complain_id=' + data["data"][i]["complain_id"] + '">详情</a></td>';
						html += '</tr>';
					}
				} else {
