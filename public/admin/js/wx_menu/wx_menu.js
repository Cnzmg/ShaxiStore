...
var DEFAULT_MENU_TIP = "一级菜单不能超过5个字节";//一级菜单名称验证提示
var DEFAULT_MENU_REGEXP = /^([\w,\u4e00-\u9fa5]){1,5}$/;//正则表达式

var DEFAULT_SUB_MENU_TIP = "二级菜单不能超过8个字节";//二级菜单名称验证提示
...
$(function() {
	...
	$(".jsMenu").live("click",function(){
		...
		//右侧编辑头部标题
		var menuname = $.trim($(this).find("span").text());
	...
	$("#pubBt").click(function(){
		if(validation()){
			$.ajax({
				url : ADMINMAIN+"/wchat/updatemenutoweixin",
				type : "post",
...
function validation(){
	...
	$("#menuList .jsMenu").each(function(){
		if(parseInt($(this).attr("data-menu-type")) == 1 && $("#urltext").val() == ""){
			flag = true;
			var element = "div[class='sub_pre_menu_box']";
			$(".jsMenu").parent().find(element).hide();//隐藏其他二级菜单
			$(".js_titleEorTips").hide();
...
function updateWeixinMenuEventType(menu_id,menu_event_type){
	$.ajax({
		url : ADMINMAIN + "/wchat/updateweixinmenueventtype",
		type : "post",
...
function addWeixinMenu(menu){
	$.ajax({
		url : ADMINMAIN + "/wchat/addweixinmenu",
		type : "post",
		data : { "menu" : menu },
...
function updateWeixinMenuName(menu_id, menu_name){
	$.ajax({
		url : ADMINMAIN + "/wchat/updateweixinmenuname",
		type : "post",
...
function updateWeixinMenuUrl(menu_id,menu_event_url){
	$.ajax({
		url : ADMINMAIN + "/wchat/updateweixinmenuurl",
		type : "post",
...
function deleteWeixinMenu(menu_id){
	...
	$.ajax({
		url : ADMINMAIN + "/wchat/deleteweixinmenu",
		type : "post",
...
function updateWeiXinMenuMessage(menu_id, media_id, menu_event_type){
	$.ajax({
		url : ADMINMAIN + "/wchat/updateweixinmenumessage",
		type : "post",
...
function updateWeixinMenuSort(menu_id_arr,flag){
	$.ajax({
		url : ADMINMAIN + "/wchat/updateweixinmenuSort",
		type : "post",
