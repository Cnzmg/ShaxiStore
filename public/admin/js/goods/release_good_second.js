...
function PackageProductInfo() {
    ...
    productViewObj.is_use = $("input[name='is_use']").val();// 开启商品审核
    productViewObj.state = $("input[name='goods_state']").val();// 商品状态
...
function showGoodsGroup(){
	if($(".goods-gruop-div").is(":hidden")){
		if($(".goods-gruop-select option").length > 0){
			$(".goods-gruop-div").show();	
			$(".span-error").hide();
		}else{			
			$(".span-error").show();
		}			
...
*选择本分组 添加标签
*/
function checkGoodsGroup(){
	var group_id = $(".goods-gruop-select").val();
	var group_name = $(".goods-gruop-select option:selected").text();
	$(".goods-gruop-select option:selected").remove();
	$("#productcategory-selected").append("<span class='label' id=" + group_id + "  group_name = "+ group_name +">" + group_name + "<i class='categoryclose'></i></span>");
	$(".goods-gruop-div").hide();	
}
