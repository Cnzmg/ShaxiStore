...
//					document.write(JSON.stringify(data));
					...
					for(var key in data){
						...
						str += '<div style="color:#E31939;height:40px;line-height:40px;padding-left:10px;background: #FAFAFA;">'+key_arr[1]+'</div>';
						for(var i=0;i<data[key].length;i++){
							var data_item = data[key][i];
							cart_id_arr.push(data_item.goods_id);
							cart_num.push(data_item.num);
							var delete_cart_id = 0;
							str += '<div class="cart-item">';
							str += '<div class="item-goods">';
							str += '<span class="p-img">';
							str += '<a href="'+shop_main+'/goods/goodsinfo.html?goodsid='+data_item.goods_id+'">';
							if(data_item["picture_info"] != null){
								str += '<img src="'+upload+'/'+data_item["picture_info"]["pic_cover_big"]+'" width="50" height="50" alt="'+data_item.goods_name+'"></a></span>';
							}else{						
								str += '<img src="'+TEMP_IMG+'/goods/default_goods_img.png" width="50" height="50" alt="'+data_item.goods_name+'"></a></span>';
							}
							str += '<div class="p-name">';
							str += '<a href="'+shop_main+'/goods/goodsinfo.html?goodsid='+data_item.goods_id+'" title="'+data_item.goods_name+'">'+data_item.goods_name+'&nbsp;'+data_item.sku_name+'</a></div>';
							str += '<div class="p-price">';
							if(data_item.point_exchange_type==1){
								str += '<strong>¥'+data_item.price+'+积分'+data_item.point_exchange+'</strong>x'+data_item.num+'</div>';
							}else{
								str += '<strong>¥'+data_item.price+'</strong>×'+data_item.num+'</div>';
							}
								str += '<a href="javascript:;" class="p-del" onclick="deleteShoppingCartById('+data_item.cart_id+')">删除</a>';
								str += '</div></div>'; 
							total += data_item.price * data_item.num;
						}
					}
					if(str != ''){
						$(".cart-panel-footer").show();
