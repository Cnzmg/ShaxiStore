...
class Goods extends BaseController
{
    ...
    public function goodsinfo()
    {
        ...
        if (isset($_GET["goodsid"])) {
            ...
            if(isset($this->uid))
            {
                $is_llegral_inform = $user_service->getUserInfoByUid($this->uid)['is_llegral_inform'];
            }
            $this->assign('is_llegral_inform',$is_llegral_inform);            
    ...
    public function goodsList()
    {
        ...
        $order = isset($_GET["order"]) ? $_GET["order"] : "";//要排序的字段
        $sort = isset($_GET["sort"]) ? $_GET["sort"] : "desc";//升序降序
    ...
    {echo \think\Request::instance()->root();die;
        $this->member = new MemberService();
