...
class Member extends BaseController
{
    ...
    public function  followGoods()
    {
        ...
        //dump($goods_collection_list["data"]);
       ...
       // $this->assign("member_detail", $member_detail);
        ...
        $list = $member->getMemberShopsFavoritesList($page, 10, $datashop);
              
        $assign_get_list = array(
            //'page' => $page, // 当前页
            'list' => $list['data'] , // 店铺列表
            'shoppage_count' => $list['page_count']// 总页数
        ...
        foreach ($assign_get_list as $key => $value) {  //循环输出，符值
       ...
       // $member_detail = $this->user->getMemberDetail($this->instance_id);
        ...
        return view($this->style . 'Member/followGoods');
     ...
     * 拼单列表页面
     */
    public function fightAll(){
        return view($this->style . 'Member/fightAll');
    }
