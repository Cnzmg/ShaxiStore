...
//             }
     ...
     * 活动管理
     */
    public function activityAdministrationList()
    {
        if (request()->isAjax()) 
        {
            $page_index = request()->post("page_index", 1);
            ...
            $activity_state = request()->post('status', '');
    
            $discount = new PromotionService();
    
            $condition = array('activity_title' => array('like','%' . $search_text . '%'));
            $activity = new Activity();
            $list = $activity ->getPlatformActivityList($page_index,$page_size,$condition,'','');
            return $list;    
        }
        ...
        $child_menu_list = array(
            array(
                'url' => "promotion/activityadministrationlist",
                'menu_name' => "活动管理",
                "active" => $status == '-1' ? 1 : 0
            ),
        );
        $this->assign('child_menu_list', $child_menu_list);   
        return view($this->style . "Promotion/activityAdministrationList");
    
    }
    /**
     * 参加活动
     */
    public function articipateActivities()
    {
        $discount = new Activity();
        $activity_id = request()->get('activity_id','');
        $shop_id = $this->instance_id;
        $list = $discount->getShopJoinPlatformActivityList($activity_id, $shop_id);
    
        if (request()->isAjax())
        {
            $discount = new Activity();
            $activity_id = request()->post('activity_id','');
            $goods_ids = request()->post('goods_id_array','');
            $shop_id = $this->instance_id;
    
            $retval = $discount->addPlatformActivityDetailBatch($activity_id, $goods_ids, $shop_id);
        ...
        return view($this->style . "Promotion/articipateActivities");
    ...
    public function setactivitystate()
    {
        if (request()->isAjax())
        {
            ...
            $retval = $Activity->setPlatformActivityDetailState($activity_detail_id, 0);
            return AjaxReturn($retval);
        }
