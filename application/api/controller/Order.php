...
class Order extends BaseController
{
    ...
    */
    public function orderCreate(){
        ...
        if(empty($uid))
            {
                return  $this->outMessage('-2001', ERROR_CODE, '当前用户未登录');
            }
       //获取用户地址
        $member = new Member();
        $address = $member->getDefaultExpressAddress();
            
        $order_type = isset($this->request_common_array['order_type']) ? $this->request_common_array['order_type'] : 1;//订单类型，默认1普通订单
        $pay_type = isset($this->request_common_array['pay_type']) ? $this->request_common_array['pay_type'] : 1;
        $shipping_type = isset($this->request_common_array['shipping_type']) ? $this->request_common_array['shipping_type'] : 1;
        ...
        $point = isset($this->request_common_array['point']) ? $this->request_common_array['point'] : 0;//使用积分
        $coupon_id = isset($this->request_common_array['coupon_id']) ? $this->request_common_array['coupon_id'] : 0;//优惠券id
        $goods_sku_list = isset($this->request_common_array['goods_sku_list']) ? $this->request_common_array['goods_sku_list'] : '';//商品列表skuid：num,skuid:num...
        $platform_money = isset($this->request_common_array['platform_money']) ? $this->request_common_array['platform_money'] : 0;//用户余额
        $pick_up_id = isset($this->request_common_array['pick_up_id']) ? $this->request_common_array['pick_up_id'] : 0;//自提点
        $shipping_company_id = isset($this->request_common_array['shipping_company_id']) ? $this->request_common_array['shipping_company_id'] : 0;//物流公司
        $out_trade_no = $this->service->getOrderTradeNo();;//支付号，进入订单之前要生成
        $order_from = 1;
        $coin = 0;//使用购物币,当前设置为0
        ...
        $shipping_time = $shipping_time = date("Y-m-d H::i:s", time());
        $res = $this->service->orderCreate($order_type, $out_trade_no, $pay_type, $shipping_type, $order_from, $buyer_ip, $buyer_message, $buyer_invoice, $shipping_time, $receiver_mobile, $receiver_province, $receiver_city, $receiver_district, $receiver_address, $receiver_zip, $receiver_name, $point, $coupon_id, 0, $goods_sku_list, $platform_money, $pick_up_id, $shipping_company_id, $coin);
        $retval = AjaxReturn($res);
        if($retval['code'] > 0){
            return $this->outMessage($retval['code']);
        }else{
