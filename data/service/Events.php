...
class Events implements IEvents{
     ...
     *普通平台活动 修改产品活动是否开始结束
     * (non-PHPdoc)
     * @see \data\api\IEvents::activityTask()
     */
    public function activityTask()
    {
        ...
        //开始活动
        $start_activity_list = $activity_service->getPlatformActivityList(1, 0, ['activity_start_date' => array('lt',$date_time), 'activity_state'=>1]);
    
        foreach($start_activity_list['data'] as $item)
        {
            $activity_service->setPlatformActivityState($item['activity_id'], 2);
        }
    
        //结束时间
        $end_activity_list = $activity_service->getPlatformActivityList(1, 0,  ['activity_end_date' => array('lt',$date_time), 'activity_state'=>2]);
    
        foreach ($end_activity_list['data'] as $item){
            $activity_service->setPlatformActivityState($item['activity_id'], 3);
        }
