...
class Goods extends BaseService implements IGoods
{
     ...
     *
     * state '商品状态 -1待审核, 0下架，1正常，10违规（禁售）',
     * sale_date '上下架时间',
     * create_time '商品添加时间',
     * update_time '商品编辑时间',
     * sort '排序',
     ...
     */
    public function addOrEditGoods($goods_id, $goods_name, $shopid, $category_id, $category_id_1, $category_id_2, $category_id_3, $supplier_id, $brand_id, $group_id_array, $goods_type, $market_price, $price, $cost_price, $point_exchange_type, $point_exchange, $give_point, $is_member_discount, $shipping_fee, $shipping_fee_id, $stock, $max_buy, $min_stock_alarm, $clicks, $sales, $collects, $star, $evaluates, $shares, $province_id, $city_id, $picture, $keywords, $introduction, $description, $QRcode, $code, $is_stock_visible, $is_hot, $is_recommend, $is_new, $sort, $image_array, $sku_array, $state, $sku_img_array, $goods_attribute_id, $goods_attribute, $goods_spec_format, $goods_weight, $goods_volume, $shipping_fee_type, $extend_category_id, $sku_picture_values)
    {
        $error = 0;
     ...
     * @see \data\api\IGoods::editGoodsOnline()
     */
    public function ModifyGoodsExam($condition)
    {
        $data = array(
            "state" => 1,
            'update_time' => date('y-m-d h:i:s', time())
 ...
 * 商品状态更改
 * (non-PHPdoc)
 * @see \data\api\IGoods::ModifyGoodsState()
 */
    public function  ModifyGoodsState($condition, $state)
    {
      if($state) 
      {
          $data = array("state" => -1, 'update_time' => date('y-m-d h:i:s', time()));
          $result = $this->goods->save($data, "goods_id  in($condition) and state in(2)");
      }
          $data = array("state" => $state,'update_time' => date('y-m-d h:i:s', time()));
          $result = $this->goods->save($data, "goods_id  in($condition) and state in(0,1)");
     ...
     */
    public function getGoodsInfo($goods_id, $condition = ''){
    
        if(!empty($goods_id)){
            $condition['goods_id'] = $goods_id;
        }
        $goods_model = new NsGoodsModel();
        $goods_info = $goods_model->getInfo($condition, '*');
        return $goods_info;
    }   
